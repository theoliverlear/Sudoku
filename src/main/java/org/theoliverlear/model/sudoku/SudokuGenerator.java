...
public class SudokuGenerator {
    ...
    public void generateBoardRandomly() {
        ...
        // than 2 seconds, then the method will recursively call itself to
    ...
    //---------------------Add-Num-To-Nums-Tried-At-Board---------------------
    ...
    public void generateValidMutedBoard(int numMuted) {
        ...
        for (int i = 0; i < numMuted;) {
            ...
            boolean containsNum = this.containsAny(randIndex.getRowIndex(),
                                                   randIndex.getColumnIndex(),
                                                   randNumber);
            if (!containsNum) {
                int row = randIndex.getRowIndex() + 1;
                int column = randIndex.getColumnIndex() + 1;
                this.sudoku.getBoard().placeNumber(row, column, randNumber);
                i++;
            }
        }
    }
    ...
    //---------------------Generate-Board-Sequentially------------------------
    @Deprecated
    public void generateBoardSequentially() {
        // This method attempts to generate a complete sudoku board by
        // implementing a backtracking algorithm. Note that most methods are
        // index-based, meaning that they start from zero, but some methods
        // like the placeNumber method are number-based, meaning that they
